...
import apijson.demo.model.Stone;
...
public class DemoSQLConfig extends APIJSONSQLConfig {
	...
	public String getDBVersion() {
		if (isMySQL()) {
			return "8.0.19"; //"8.0.11"; //TODO 改成你自己的 MySQL 或 PostgreSQL 数据库版本号 //MYSQL 8 和 7 使用的 JDBC 配置不一样
		}
		if (isPostgreSQL()) {
	...
	public String getDBUri() {
		if (isMySQL()) {
			...
			// 以下是 MySQL 5.7 及以下
			//return "jdbc:mysql://127.0.0.1:3306?serverTimezone=GMT%2B8&useUnicode=true&characterEncoding=UTF-8"; //TODO 改成你自己的，TiDB 可以当成 MySQL 使用，默认端口为 4000
		}
		if (isPostgreSQL()) {
